...
//
//public:
//	SharedPtr(T* ptr = nullptr, DelFuncPtr del = nullptr): ptr_(ptr), count_ptr_(new size_t(ptr != nullptr)), del_(del){}
//
//	~SharedPtr()
//	{
//		if (!ptr_) return;
...
//	}
//
//	SharedPtr(const SharedPtr& sp) : ptr_(sp.ptr_), count_ptr_(sp.count_ptr_), del_(sp.del_){++*count_ptr_;}
//
//	SharedPtr& operator=(SharedPtr sp){	swap(sp);return *this;}
//
//	SharedPtr(const SharedPtr&& sp) noexcept : SharedPtr() { swap(sp); }
//
//	void reset(T* ptr = nullptr, DelFuncPtr del = nullptr){SharedPtr tmp(ptr, del);swap(tmp);}
//
